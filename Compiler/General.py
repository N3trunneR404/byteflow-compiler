...
"""
This file holds functions that generate general Byteflow code
...
def process_switch_cases(expression_code, cases):
    ...
    """
        ...
        It does so by adjusting the expression value and checking if it becomes zero.
        ...
        The expression value (x) will be placed in the <expression> cell, then:
        Iteration 1 will adjust the <expression> cell by -1 (comparing x with 1).
        Iteration 2 will adjust the <expression> cell by -4 (comparing x with 5).
        Iteration 3 will adjust the <expression> cell by +5 (comparing x with 0).
        Iteration 4 will adjust the <expression> cell by -2 (comparing x with 2).
    """
    ...
    Each case's '[' instruction uses the next case's ']' instruction to return to the comparisons block.
    For example, the '[' in the case 5 line uses the ']' of the case 1 code to "return" to the comparisons.
    This is because there is no way to "skip" code directly in Byteflow.
    """
...
def get_divmod_code(right_token=None):
    ...
    def get_if_equal_to_0_code(inside_if_code, offset_to_temp_cell):
        """
        Wraps the given <inside_if_code> with an "if (current_cell == 0) {<inside_if_code>}" condition.

        During the process, it zeros the current cell and uses a temporary cell.
        The argument <offset_to_temp_cell> specifies the offset from the current cell to the temporary cell.
        *** Note that the temporary cell must be AFTER the cells that the <inside_if_code> modifies. ***

        <inside_if_code> should assume it starts running when pointing to the current cell
        and it should end its execution pointing to the same cell.
        """
